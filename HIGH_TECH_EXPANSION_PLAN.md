# GURPS High-Tech Expansion Plan

## Status: IN PROGRESS
**Started:** 2026-05-03
**Goal:** Add all items from GURPS High-Tech (TL5-8) to valinoreth

## Completion Status

### Overall Progress
- ✅ Basic Set (BS) - COMPLETE
- ✅ Low-Tech (LT) - COMPLETE
- 🔄 High-Tech (HT) - IN PROGRESS

### High-Tech Progress by Category
- 🔄 Ranged Weapons - IN PROGRESS (121/~150+ items)
- ⬜ Melee Weapons - NOT STARTED
- ⬜ Armor - NOT STARTED
- ⬜ Clothing - NOT STARTED
- ⬜ Survival Gear - NOT STARTED
- ⬜ Tools - NOT STARTED
- ⬜ Containers - NOT STARTED

## Phase 1: TL5 Ranged Weapons (Industrial Revolution, 1730-1880)

### Completed Batches
- ✅ Batch 1: Early Pistols (Cap-lock, Pepperbox, DA/SA Revolvers, etc.)
- ✅ Batch 2: Early Rifles (Henry, Lever-action, Martini-Henry, etc.)
- ✅ Batch 3: Heavy Weapons & Explosives (Gatling, Maxim, Mitrailleuse, Dynamite, TNT)
- ✅ Batch 4: TL5 Shotguns (Double-barrel, Pump-action, Winchester 1887)
- ✅ Batch 5: TL5 Pistols (Colt Walker, Remington Rolling Block, etc.)
- ✅ Batch 6: TL5 Rifles (Chassepot, Dreyse Needle, Enfield Pattern 1853, etc.)
- ✅ Batch 7: TL5 Artillery & Support (Field Gun, Mountain Gun, Naval Gun, etc.)
- ✅ Batch 8: TL6 Pistols (Colt M1911, Luger P08, Mauser C96, etc.)
- ✅ Batch 9: TL6 Rifles (Lee-Enfield SMLE, M1 Garand, Mauser Gewehr 98, etc.)
- ✅ Batch 10: TL6 SMGs (Thompson M1928, MP40, Sten, PPSh-41, etc.)
- ✅ Batch 11: TL6 Machine Guns (BAR, Bren, Browning M2, MG34, MG42, etc.)
- ✅ Batch 12: TL6 Shotguns & Specialized (Browning Auto-5, Winchester 1897, Boys ATR, PTRD ATR, etc.)
- ✅ Batch 13: TL7 Pistols (Beretta M92, Glock 17, Desert Eagle, CZ 75, SIG P220, Makarov PM)
- ✅ Batch 14: TL7 Rifles (AK-47, AKM, M14, M16, FN FAL, G3)
- ✅ Batch 15: TL7 SMGs (MAC-10, MP5, Skorpion vz. 61, Sterling, Uzi)
- ✅ Batch 16: TL7 Machine Guns (FN MAG, M249 SAW, M60, MG3, PKM)
- ✅ Batch 17: TL7 Sniper Rifles (Barrett M82, Dragunov SVD, L96A1, M21 SWS, PSG1)
- ✅ Batch 18: TL7 Shotguns (Benelli M3, Mossberg 500, Remington 870, Saiga-12, SPAS-12)
- ✅ Batch 19: TL7 Grenade Launchers (GP-25, M203, M79, Mk 19)
- ✅ Batch 20: TL7 Explosives & Missiles (C4, Claymore, AT4, LAW M72, RPG-7, Stinger, TOW)
- ✅ Batch 21: TL8 PDWs & Modern Firearms (FN P90, HK MP7, FN Five-seveN, HK416, FN SCAR-L)

### TL5 Remaining Ranged Weapons

#### Pistols (TL5)
- [ ] Colt Walker (1847)
- [ ] Remington Rolling Block Pistol
- [ ] Webley RIC Revolver
- [ ] Smith & Wesson Model 1
- [ ] Smith & Wesson Model 3 (Schofield)

#### Rifles (TL5)
- [ ] Enfield Pattern 1853
- [ ] Chassepot Rifle
- [ ] Dreyse Needle Rifle
- [ ] Remington Rolling Block Rifle
- [ ] Winchester Model 1866
- [ ] Winchester Model 1873
- [ ] Spencer Repeating Rifle
- [ ] Trapdoor Springfield

#### Shotguns (TL5)
- [ ] Double-barrel shotgun (break-action)
- [ ] Pump-action shotgun (early)
- [ ] Winchester Model 1887 lever-action shotgun

#### Machine Guns & Artillery (TL5)
- [ ] Gardner Gun
- [ ] Nordenfelt Gun
- [ ] Mountain Gun
- [ ] Field Gun
- [ ] Naval Gun

#### Explosives & Ordnance (TL5)
- [ ] Black Powder Grenade
- [ ] Shrapnel Shell
- [ ] Canister Shot
- [ ] Grapeshot

## Phase 2: TL6 Ranged Weapons (Mechanized Age, 1880-1940)

### Pistols (TL6)
- [ ] Colt M1911
- [ ] Luger P08
- [ ] Mauser C96
- [ ] Webley Mk VI
- [ ] Browning Hi-Power
- [ ] Walther PP/PPK
- [ ] Tokarev TT-33
- [ ] Nambu Type 14

### Rifles (TL6)
- [ ] Lee-Enfield SMLE
- [ ] Mauser Gewehr 98
- [ ] Springfield M1903
- [ ] Mosin-Nagant
- [ ] Arisaka Type 38
- [ ] M1 Garand
- [ ] SVT-40
- [ ] Gewehr 43

### Submachine Guns (TL6)
- [ ] Thompson M1928
- [ ] MP18
- [ ] MP40
- [ ] Sten Gun
- [ ] PPSh-41
- [ ] Grease Gun M3
- [ ] PPS-43

### Machine Guns (TL6)
- [ ] Browning M1917
- [ ] Browning M2 .50 cal
- [ ] MG34
- [ ] MG42
- [ ] Bren Gun
- [ ] Lewis Gun
- [ ] Vickers Machine Gun
- [ ] BAR (Browning Automatic Rifle)

### Shotguns (TL6)
- [ ] Winchester Model 1897 (Trench Gun)
- [ ] Browning Auto-5
- [ ] Remington Model 31

### Specialized Weapons (TL6)
- [ ] Anti-Tank Rifle (Boys, PTRD, Panzerbüchse)
- [ ] Grenade Launcher (rifle-mounted)
- [ ] Flare Gun

### Explosives & Ordnance (TL6)
- [ ] Mills Bomb (Hand Grenade)
- [ ] Stick Grenade (Stielhandgranate)
- [ ] Pineapple Grenade (Mk 2)
- [ ] Smoke Grenade
- [ ] Mortar (various sizes)
- [ ] Bazooka
- [ ] Panzerfaust
- [ ] Panzerschreck

## Phase 3: TL7 Ranged Weapons (Nuclear Age, 1940-1980)

### Pistols (TL7)
- [ ] Beretta M92
- [ ] CZ 75
- [ ] SIG P220
- [ ] Glock 17
- [ ] Desert Eagle
- [ ] Makarov PM

### Rifles (TL7)
- [ ] AK-47
- [ ] AKM
- [ ] M16
- [ ] M14
- [ ] FN FAL
- [ ] G3
- [ ] SKS
- [ ] AR-10

### Submachine Guns (TL7)
- [ ] Uzi
- [ ] MAC-10
- [ ] MP5
- [ ] Sterling
- [ ] Skorpion vz. 61

### Machine Guns (TL7)
- [ ] M60
- [ ] M249 SAW
- [ ] PKM
- [ ] FN MAG
- [ ] MG3

### Sniper Rifles (TL7)
- [ ] Dragunov SVD
- [ ] M21 SWS
- [ ] PSG1
- [ ] L96A1
- [ ] Barrett M82

### Shotguns (TL7)
- [ ] Remington 870
- [ ] Mossberg 500
- [ ] SPAS-12
- [ ] Benelli M3
- [ ] Saiga-12

### Grenade Launchers (TL7)
- [ ] M79
- [ ] M203 (underbarrel)
- [ ] GP-25 (underbarrel)
- [ ] Mk 19 (automatic)

### Explosives & Missiles (TL7)
- [ ] C4 Explosive
- [ ] Claymore Mine
- [ ] RPG-7
- [ ] AT4
- [ ] LAW (M72)
- [ ] Stinger Missile
- [ ] TOW Missile

## Phase 4: TL8 Ranged Weapons (Digital Age, 1980-2025)

### Modern Firearms (TL8)
- [ ] Modern assault rifles (various)
- [ ] PDWs (P90, MP7)
- [ ] Modern precision rifles
- [ ] Advanced grenade systems
- [ ] Smart munitions
- [ ] Railgun prototypes
- [ ] Coilgun prototypes
- [ ] Laser weapons (experimental)

## Phase 5: High-Tech Melee Weapons

### TL5-6 Melee
- [ ] Bayonets (various)
- [ ] Trench knives
- [ ] Combat knives
- [ ] Entrenching tools (combat use)

### TL7-8 Melee
- [ ] Modern combat knives
- [ ] Tactical tomahawks
- [ ] Stun batons
- [ ] Monofilament weapons
- [ ] Vibro-blades

## Phase 6: High-Tech Armor

### TL5 Armor
- [ ] Early bulletproof vests (experimental)
- [ ] Steel breastplates (cavalry)

### TL6 Armor
- [ ] Steel helmets (WWI/WWII)
- [ ] Flak jackets
- [ ] Tank crew armor

### TL7 Armor
- [ ] Kevlar vests (various levels)
- [ ] PASGT helmet
- [ ] Ballistic plates
- [ ] Bomb suit
- [ ] Riot gear

### TL8 Armor
- [ ] Modern plate carriers
- [ ] FAST helmets
- [ ] Advanced composite armor
- [ ] Powered armor (experimental)

## Phase 7: Other High-Tech Equipment

### Clothing
- [ ] Military uniforms (various eras)
- [ ] Camouflage patterns
- [ ] NBC suits
- [ ] Ghillie suits
- [ ] Flight suits
- [ ] Pressure suits

### Survival Gear
- [ ] Modern camping equipment
- [ ] GPS devices
- [ ] Night vision goggles
- [ ] Thermal imaging
- [ ] Emergency radios
- [ ] Water purification tablets
- [ ] MREs
- [ ] Emergency blankets

### Tools
- [ ] Modern toolkits
- [ ] Electronic tools
- [ ] Lockpicking sets (modern)
- [ ] Bolt cutters
- [ ] Plasma cutters
- [ ] Power tools

### Containers
- [ ] Modern backpacks (tactical)
- [ ] Ammo boxes
- [ ] Pelican cases
- [ ] Duffel bags
- [ ] Plate carriers (as containers)

## Implementation Notes

### Current Approach
1. Working through ranged weapons systematically by TL
2. TL5 being completed in batches (pistols → rifles → heavy weapons)
3. Each batch includes complete implementation across all methods

### Code Requirements
Each weapon must implement:
- `base_cost()` - Currency value
- `weight()` - Weight in pounds
- `tech_level()` - TechLevel
- `damage()` - WeaponDamage (dice + type)
- `accuracy()` - i32 accuracy bonus
- `required_skill()` - Skill enum

### Testing & Quality
- ✅ All code must pass `cargo check`
- ✅ All code must pass `cargo clippy`
- ✅ All code must pass `cargo test`
- ✅ Zero warnings policy

### Commit Strategy
- One batch per commit
- Descriptive commit messages following project style
- Format: `feat(items): Add [Category] - High-Tech Phase X Batch Y`

## Resources

### Primary Sources
- GURPS High-Tech (4th Edition)
- GURPS High-Tech: Adventure Guns
- GURPS WWII: Motor Pool
- GURPS WWII: Iron Cross
- GURPS Ultra-Tech (for TL8+ boundary items)

### Reference Data Needed
For each weapon:
- Damage dice (from GURPS tables)
- Weight (lbs)
- Cost ($)
- Accuracy modifier
- Range
- Rate of Fire
- Shots capacity
- Skill requirement
- Tech Level

## Next Steps

1. ✅ Complete TL5 remaining weapons
2. Move to TL6 pistols and rifles
3. Add TL6 SMGs and machine guns
4. Progress through TL7 and TL8
5. Move to melee weapons, armor, equipment

## Estimated Scope

- **Ranged Weapons:** ~150-200 items
- **Melee Weapons:** ~30-50 items
- **Armor:** ~40-60 items
- **Other Equipment:** ~100-150 items
- **Total:** ~350-500 High-Tech items

## Progress Tracking

### Completed: 121 items
- 13 TL5 Pistols
- 16 TL5 Rifles
- 3 TL5 Shotguns
- 8 TL5 Machine Guns & Artillery
- 6 TL6 Pistols
- 6 TL6 Rifles
- 7 TL6 SMGs
- 8 TL6 Machine Guns
- 3 TL6 Shotguns
- 3 TL6 Specialized Weapons
- 6 TL7 Pistols
- 6 TL7 Rifles
- 5 TL7 SMGs
- 5 TL7 Machine Guns
- 5 TL7 Sniper Rifles
- 5 TL7 Shotguns
- 4 TL7 Grenade Launchers
- 7 TL7 Explosives & Missiles
- 5 TL8 PDWs & Modern Firearms

### Remaining: ~229-379 items

---

**Last Updated:** 2026-05-06
**Status:** 🔄 TL8 PDWs & Modern Firearms Batch 21 COMPLETE - Phase 4 (TL8) IN PROGRESS
